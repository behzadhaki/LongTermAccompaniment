...
import torch
from model import Groove2GrooveVAE
from helpers import train_utils, eval_utils_g2g
from data.src.dataLoaders import Groove2Groove2BarDataset
...
# ----------------------- Data Parameters -----------------------
parser.add_argument("--bz2dataset_path", type=str,
                    help="Path to the dataset file in bz2pickle format")
...
if args.config is not None:
    ...
    hparams = dict(
        ...
        bz2dataset_path=args.bz2dataset_path,
...
if __name__ == "__main__":
    ...
    wandb_run = wandb.init(
        ...
        settings=wandb.Settings(code_dir="train_G2G.py")  # for code saving
    )
    ...
    if loaded_via_config:
        ...
        model_code.add_file("train_G2G.py")
    ...
    training_dataset = Groove2Groove2BarDataset(
        dataset_bz2_filepath=config.bz2dataset_path,
        subset_tag="train",
        max_len=int(args.max_len_enc),
        tapped_voice_idx=2,
        use_cached=True,
        down_sampled_ratio=None if args.is_testing is True else None,
        augment_dataset=True,
        move_all_to_gpu=should_place_all_data_on_cuda
    )
    train_dataloader = DataLoader(training_dataset, batch_size=config.batch_size, shuffle=True)

    test_dataset = Groove2Groove2BarDataset(
        dataset_bz2_filepath=config.bz2dataset_path,
        subset_tag="test",
        max_len=int(args.max_len_enc),
        tapped_voice_idx=2,
    ...
    # ------------------------------------------------------------------------------------------------------------
    model_cpu = Groove2GrooveVAE(config)
    ...
    def batch_data_extractor(data_, device=config.device):
        ...
        inputs_grooves = data_[0].to(device) if data_[0].device.type != device else data_[0]
        outputs_grooves = data_[1].to(device) if data_[1].device.type != device else data_[1]
        tempos = data_[2].to(device) if data_[2].device.type != device else data_[2]
        genre_encodings = data_[3].to(device) if data_[3].device.type != device else data_[3]
        genre_targets = data_[4].to(device) if data_[4].device.type != device else data_[4]
        return inputs_grooves, outputs_grooves, tempos, genre_encodings, genre_targets


    def predict_using_batch_data(batch_data, model_=model_on_device, device=config.device):
        inputs_grooves, outputs_grooves, _, _, _ = batch_data_extractor(batch_data, device)
        with torch.no_grad():
            hvo, latent_z = model_.predict(flat_hvo_groove=inputs_grooves)
        return hvo, latent_z
