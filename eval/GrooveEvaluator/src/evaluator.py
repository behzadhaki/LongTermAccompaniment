...
class Evaluator:
    ...
    def __init__(
        ...
        :param hvo_sequences_list_: A list of HVO_Sequence objects (if dataset is not provided)
        :param dataset: (Default: None) A dataset object that contains the metadata for the HVO_Sequence objects (if hvo_sequences_list_ is not provided)
        ...
        """
        assert hvo_sequences_list_ is not None or dataset is not None, "Either hvo_sequences_list_ or dataset must be provided"
        ...
        print(self._gt_hvo_sequences[0].metadata)

        self.full_midi_filename_to_ix = {hvo_seq.metadata['full_midi_filename']: ix for ix, hvo_seq in enumerate(self._gt_hvo_sequences)}
    ...
    def __setstate__(self, state):
        ...
        self.full_midi_filename_to_ix = {hvo_seq.metadata['full_midi_filename']: ix for ix, hvo_seq in
                                         enumerate(self._gt_hvo_sequences)}
    ...
    def get_pos_neg_hit_scores(self, hit_weight=1, return_as_pandas_df=False):
        ...
        for (true_values, predictions) in zip(self._gt_hvos_array, prediction_hvos_array):
            ...
            if predictions is not None:
                ...
                JACCARD.append(TP / (TP + FP + FN) if (TP + FP + FN) > 0 else 0)
    ...
    def add_unsorted_predictions(self, prediction_hvos_array, prediction_full_midi_filenames):

        sorted_array = np.zeros_like(prediction_hvos_array)

        # get corresponding indices in the _gt_** arrays
        for i, path in enumerate(prediction_full_midi_filenames):
            correct_ix = self.full_midi_filename_to_ix[path]
            sorted_array[correct_ix] = prediction_hvos_array[i]

        self.add_predictions(sorted_array)
    ...
    def dump(self, path=None, fname="evaluator"):  # todo implement in comparator
        ...
        path_to_save = self.get_dump_path(path=path, fname=fname)

        if not path_to_save.endswith(".Eval.bz2"):
            path_to_save += ".Eval.bz2"
        if not os.path.exists(os.path.dirname(path_to_save)):
            os.makedirs(os.path.dirname(path_to_save), exist_ok=True)

        ofile = bz2.BZ2File(path_to_save, 'wb')
        pickle.dump(self, ofile)
        ofile.close()

        print(f"Dumped Evaluator to {os.path.join(path, f'{self._identifier}_{fname}')}")

    def get_dump_path(self, path=None, fname="evaluator"):
        if path is None:
        ...
        return os.path.join(path, f"{self._identifier}_{fname}")
