...
import os
from data import Groove2Drum2BarDataset, Groove2Groove2BarDataset
import logging
import numpy as np
...
def create_template(dataset, identifier,
                    cached_folder="cached/Evaluators/templates/", divide_by_genre=True, max_hvo_shape=(32, 27)):
    """
    ...
    # load data using the settings specified in the dataset_bz2_filepath
...
def load_g2g_evaluator_template(dataset_bz2_filepath, subset_name,
                            down_sampled_ratio, cached_folder="cached/Evaluators/templates/",
                            divide_by_genre=True, disable_logging=True):
    """
    Load a template for the given dataset and subset. If the template does not exist, it will be created and
    ...
    :param dataset_bz2_filepath:  The path to the json file that contains the dataset settings.
    ...
    :return:                        The evaluator template.
    """
    dataset_name = dataset_bz2_filepath.split("/")[-1].split(".")[0]
