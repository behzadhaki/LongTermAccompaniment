...
#
#
# class Groove2Groove2BarDataset(Dataset):
#     def __init__(self, dataset_bz2_filepath, subset_tag, max_len, tapped_voice_idx=2,
...
#
#         """
#         :param dataset_bz2_filepath:  path to the bz2 file containing the dataset settings
#         :param subset_tag:                [str] whether to load the train/test/validation set
#         :param max_len:              [int] maximum length of the sequences to be loaded
#         :param tapped_voice_idx:    [int] index of the voice to be tapped (default is 2 which is usually closed hat)
#         :param down_sampled_ratio: [float] down samples the data by the ratio provided (e.g. 0.5)
...
#         :param force_regenerate: [bool] if True, will regenerate the cached version of the dataset
#         """
#         self.dataset_setting_json_path = dataset_bz2_filepath
#
#         if down_sampled_ratio is not None:
#             if down_sampled_ratio <= 0:
#                 down_sampled_ratio = None
#
#             assert down_sampled_ratio < 1, "down_sampled_ratio should be less than 1"
#
#         def get_cached_filepath():
#             dir_ = "cached/TorchDatasets"
#             filename = (f"{dataset_bz2_filepath.split('/')[-1]}_{subset_tag}_{max_len}_{tapped_voice_idx}"
#                         f"_{augment_dataset}_{down_sampled_ratio}.bz2pickle")
#             if not os.path.exists(dir_):
#                 os.makedirs(dir_)
#             return os.path.join(dir_, filename)
#
...
#         if use_cached and not force_regenerate:
#             if os.path.exists(get_cached_filepath()):
#                 dataLoaderLogger.info(f"Groove2Groove2BarDataset Constructor --> Loading Cached Version from: {get_cached_filepath()}")
#                 ifile = bz2.BZ2File(get_cached_filepath(), 'rb')
#                 data = pickle.load(ifile)
#                 ifile.close()
#                 self.input_grooves = torch.tensor(data["input_grooves"], dtype=torch.float32)
...
#             #   a portion of them uniformly sampled if down_sampled_ratio is provided
#             # ------------------------------------------------------------------------------------------
#             with bz2.BZ2File(dataset_bz2_filepath, 'rb') as f:
#                 assert os.path.exists(dataset_bz2_filepath), f"Dataset Setting File Not Found: {dataset_bz2_filepath}"
#                 loaded_data = pickle.load(f)
#                 subset = loaded_data[subset_tag]
#
#             if subset_tag == "train":
#                 self.genre_tags = list(sorted(set(subset['style_primary'])))
#             else:
#                 with bz2.BZ2File(dataset_bz2_filepath, 'rb') as f:
#                     loaded_data = pickle.load(f)
#                     subset_temp = loaded_data["train"]
#                 self.genre_tags = list(sorted(set(subset_temp['style_primary'])))
#
#             self.num_genres = len(self.genre_tags)
#             # create a one-hot encoding for each genre
#             self.genre_mapping = {genre: np.eye(self.num_genres)[idx].tolist() for idx, genre in enumerate(self.genre_tags)}
...
#             # ------------------------------------------------------------------------------------------
#
#             for idx, hvo_seq in enumerate(tqdm(subset['outputs_hvo_seqs'], desc="loading data and extracting input output tensors")):
#                 self.hvo_sequences.append(hvo_seq)
#                 hvo_seq.adjust_length(max_len)
#                 self.tempos.append(hvo_seq.tempos[0].qpm)
#                 self.genre_encodings.append(self.map_genre_to_one_hot(hvo_seq.metadata["style_primary"]))
#                 self.genre_targets.append(self.genre_tags.index(hvo_seq.metadata["style_primary"]))
#
#             self.tempos = torch.tensor(np.array(self.tempos), dtype=torch.long)
#
#             # Load as tensors
#             self.input_grooves = torch.tensor(np.array(subset['inputs']), dtype=torch.float32)
#             self.output_grooves = torch.tensor(np.array(subset['outputs']), dtype=torch.float32)
#             self.genre_encodings = torch.tensor(np.array(self.genre_encodings), dtype=torch.float32)
#             self.genre_targets = torch.argmax(self.genre_encodings, dim=1)
#
#             groove_hit_counts = self.input_grooves[:, :, 0].sum(dim=1)
#             output_groove_counts = self.output_grooves[:, :, 0].sum(dim=1)
#             ratio = output_groove_counts / groove_hit_counts
#             ratio = ratio.numpy()
#             print(f"Max Ratio of output_groove_counts / groove_hit_counts: {ratio.max()}, shape: {ratio.shape}")
#
#             # check if should down sample the data
#             if down_sampled_ratio is not None:
#                 n_samples = int(len(self.input_grooves) * down_sampled_ratio)
#                 idxs = np.random.choice(len(self.input_grooves), n_samples, replace=False)
#                 self.input_grooves = self.input_grooves[idxs]
#                 self.output_grooves = self.output_grooves[idxs]
#                 self.genre_encodings = self.genre_encodings[idxs]
#                 self.genre_targets = self.genre_targets[idxs]
#                 self.tempos = self.tempos[idxs]
#                 self.hvo_sequences = [self.hvo_sequences[idx] for idx in idxs]
#
#             if augment_dataset:
#                 # augment the dataset by appending the bar-swapped version of each sequence
#                 self.hvo_sequences = self.hvo_sequences * 2
...
#     def __getitem__(self, idx):
#         return (self.input_grooves[idx],    # 0: input_groove
#                 self.output_grooves[idx],   # 1: output_groove (drum hvo)
#                 self.tempos[idx],           # 3: tempo (real value - not binned)
#                 self.genre_encodings[idx],  # 4: genre_encoding (one-hot)
#                 self.genre_targets[idx]     # 5: genre_target (categorical)
#                 )
#     def __repr__(self):
#         text =  f"    -------------------------------------\n"
#         text += "Dataset Loaded using json file: \n"
        ...
        if should_process_data:
            ...
            for song_id in song_ids_all:
                ...
                if len(i1.time_signatures) == 1 and len(i2.time_signatures) == 1:
                    if i1.time_signatures[0].numerator == i2.time_signatures[0].numerator and i1.time_signatures[
                        0].denominator == i2.time_signatures[0].denominator:
