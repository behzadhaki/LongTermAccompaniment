...
if __name__ == "__main__":
    ...
    wandb_run = wandb.init(
        ...
        settings=wandb.Settings(code_dir="scripts_archived/train_LTA_1bar_lookahead.py"),
    ...
    if loaded_via_config_yaml:
        ...
        model_code.add_file("train_LTA_1bar_lookahead.py")
    ...
    def batch_data_extractor(data_, device=config.device):
        ...
        if not config['StepEncoder']['has_velocity']:
            bass_solo[:, :, n_bass_voices:2*n_bass_voices] = 0.0            # set vels to 0
        if not config['StepEncoder']['has_offset']:
